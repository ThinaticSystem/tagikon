...
| `packages/storage-adapter-drizzle/src/adapter.ts`           | `DrizzleStorageAdapter<TTag>` — `implements StorageAdapterSetup<TTag>`。コンストラクタは `(db, schema)`。`initialize(tagShape)` で IdProvider + per-property codec を一括注入して `StorageAdapter<TTag>` を返す。`safeJsonParse` で `__proto__` 等の危険キーを DB 読み取り時に除去。`#serializeTagProps` / `#deserializeTagProps` で codec を適用。`getAuxStore(extensionId, auxCodec?)` で `AuxCodec` を受け取り分岐。`findObjects` / `countObjects` で `serializePropertyValue` を query-compiler に渡す |
...
| `packages/core/src/plugin/storage-adapter/codec.ts`（更新） | `OptionalTagPropertyCodec<TValue, TStored>` インターフェース追加（`_optional: true` マーカー + `optional()` チェーン）。`tpc` 各メソッドが `.optional()` を持つ `TpcResult` 型を返すように更新。`TagFromShape<TShape>` が `_optional: true` コーデックをオプショナルプロパティ、それ以外を必須プロパティとして推論するよう更新（`id` を交差型から分離して `extends Tag` の証明を維持）。`TagShape<TTag>` の value 型に `NonNullable<...>` を適用。                                                         |

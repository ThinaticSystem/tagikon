...
1. **Utility の凝集整理** — 各モジュールに散らばっているユーティリティ関数を、ユーティリティ用の npm パッケージにまとめる

   分割の基準はこのライブラリ固有の処理ではなく JavaScript/TypeScript 製のソフトウェアなら適用し得る一般的な処理であること

2. **テストの網羅性の向上** — 現状は happy path のみのテストが多い。エラーケース（例: 存在しないタグ ID で `deleteTag` を呼ぶ）や境界値（例: `findObjects` の limit 0 / 1 / 1000）なども網羅する

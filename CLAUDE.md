...
const tag = await server.addTag("foo"); // tag は TagWithDescription として型付け
```
...
各オペレーションにつき以下4フェーズのフックが存在する（`addTag`を例に）:
...
- `findTagByName` はインターフェースに含めない。name一意性チェックは `createTag` 実装の責務

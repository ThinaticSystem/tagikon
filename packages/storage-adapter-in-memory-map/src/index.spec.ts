...
suite("MapStorageAdapter", () => {
	suite("createTag", () => {
		test("creates a tag and assigns an id", async () => {
			const adapter = new MapStorageAdapter<TagWithName>().initialize({ id: UUID_ID_PROVIDER });
	...
	suite("getTag", () => {
		test("returns the tag by id", async () => {
			const adapter = new MapStorageAdapter<Tag<Uuid>>().initialize({ id: UUID_ID_PROVIDER });
